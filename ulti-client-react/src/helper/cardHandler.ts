export class CardHandler {
	...
	public static getOrderedHand(cards: Array<number>, isColorOrder: boolean): Array<number> {
		...
		if (!isColorOrder) {
			...
			for (let i = 0; i < cards.length; i++) {
				...
				if (cards[i] == 3) {
					orderedCards = orderCards(orderedCards, i, 4);
				}

				if (cards[i] == 11) {
					orderedCards = orderCards(orderedCards, i, 12);
				}

				if (cards[i] == 19) {
					orderedCards = orderCards(orderedCards, i, 20);
				}

				if (cards[i] == 27) {
					orderedCards = orderCards(orderedCards, i, 28);
				}
			}
		}
...
function orderCards(cards: Array<number>, index: number, checkIndex: number): Array<number> {
	...
	if ((index + 1 < cards.length) && (cards[index + 1] == checkIndex
		|| cards[index + 1] == checkIndex + 1
		|| cards[index + 1] == checkIndex + 2)) {

		var tmp = cards[index];
		delete cards[index];
		cards.splice(index + 1, tmp);
	}

	if ((index + 2 < cards.length) && (cards[index + 2] == checkIndex + 2
		|| cards[index + 2] == checkIndex + 2)) {

		var tmp = cards[index + 1];
		delete cards[index + 1];
		cards.splice(index + 2, tmp);
	}

	if ((index + 3 < cards.length) && cards[index + 3] == checkIndex + 3) {

		var tmp = cards[index + 2];
		delete cards[index + 2];
		cards.splice(index + 3, tmp);
	}
