...
import hu.ulti.server.modelOld.CallWithValue;
...
import hu.ulti.server.modelOld.UuidWithCardId;
...
public class HelperOld {
	...
	public static List<Card> addTalon(Player player, List<Card> talon) {
		...
		HelperOld.orderHand(player.getHand());
	...
	public static boolean isSzintelenByList(List<Call> callList) {
		return (HelperOld.isBetlibyList(callList) || HelperOld.isSzintelenDuribyList(callList));
	}
	...
	public static boolean isSzintelenbyId(int callId) {
		return (HelperOld.isBetliById(callId) || HelperOld.isSzintelenDuriById(callId));
	}
